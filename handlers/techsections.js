...
exports.getTechsection = async function (req, res, next) {
  try {
    let foundTechsection = await db.Techsection.findById(
...
exports.updateTechsection = async function (req, res, next) {
  try {
    let techSection = await db.Techsection.findByIdAndUpdate(
...
exports.deleteTechsection = async function (req, res, next) {
  try {
    let foundTechsection = await db.Techsection.findByIdAndRemove({
      _id: req.params.techsectionId,
    });
    return res.status(200).json(foundTechsection);
  } catch (err) {
